...
use crate::message::message::Message::{ClipboardMessage, PairCodeResponseMessage};
...
impl Message {
    ...
    pub fn check_entire_message(src: &mut Cursor<&[u8]>) -> AppResult<()> {
        if !src.has_remaining() {
            ...
            if src.remaining() < 5 {
                ...
                if len <= remain_size { //等于的时候刚好是一个完整的消息，大于的时候说明有粘包
    ...
    pub fn encode(self) -> BytesMut {
        ...
        if content.len()>Self::MAX_SIZE {
            ...
            let text= "the content is too long,i have ignore it.".to_string();
            let message = ClipboardMessage(text);
            let content = serde_json::to_vec(&message).unwrap();
            bytes_mut.put_u8(message.get_tye_id());
...
fn test_encode() {
    ...
    // let message = Message::HeartPackageMessage();
    let message = Message::ClipboardMessage("hello你好123".into());
    let socket = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 8080);
    // let message1 = Message::PairRequestMessage("test".into(), DeviceInfo::new(socket, "xbss".into()));
    let message = Message::PairDeviceInfosResponseMessage(vec![DeviceInfo::new(socket, "xbss".into()),DeviceInfo::new(socket, "xbss".into())]);
    // let message = Mssage::PairCodeResponseMessage("in".into());
    // let message = Message::DeviceChangeResponseMessage(true,DeviceInfo::new(socket, "xbss".into()));
    // let string = serde_json::to_string(&message).unwrap();
    let string = serde_json::to_string(&message).unwrap();
    println!("{string}");
    println!("{:?}",message.encode().as_ref());
